...
const GAME_SETTINGS = {
  ...
  symbolPathPrefix: "/images/moon/",
  images: {
    spin: "/images/moonmission/spin.png",
    autoSpin: "/images/moonmission/autospin.png",
    buyBonus: "/images/moonmission/buybonus.png",
    pageBg: "/images/moonmission/yehey.png",
    cardBg: "/images/moonmission/pagebg.png",
    gridBg: "/images/moonmission/grid.png",
    yeheyBg: "/images/moonmission/yehey.png",
  },
};
...
const getSymbolPath = (filename: string) => {
  const cleanFilename = filename.startsWith("moon/") ? filename.replace("moon/", "") : filename;
  return `${GAME_SETTINGS.symbolPathPrefix}${cleanFilename}`;
};

class MoonMissionAudio {
...
const soundFx = new MoonMissionAudio();

export default function MoonMissionPage() {
...
// --- RESTORE GAME STATE FUNCTION ---
  const restoreGameState = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/spin/moonmission?walletAddress=${publicKey.toBase58()}`);
      const data = await res.json();
  ...
  const [grid, setGrid] = useState<SymbolCell[][]>(() =>
    Array(5).fill(null).map((_, col) => Array(3).fill(null).map((_, row) => ({ id: `ssr-${col}-${row}`, symbol: "rug.png", isGold: false, multiplier: 0, isWild: false, isScatter: false })))
  );
  ...
  const executeSpin = async (buyType?: "standard" | "super" | null) => {
    ...
    try {
      const response = await fetch("/api/spin/moonmission", {
        method: "POST",
                  ...
                  col.map((cell, rowIdx) => {
                    ...
                    return (
                      <div
                        ...
                        isWinningCell
  ? "ring-2 sm:ring-4 ring-amber-300 bg-amber-500/40 scale-105 z-30 animate-pulse shadow-[0_0_18px_rgba(245,158,11,0.9)]"
  : cell.isGold
